...
if (-not $WheelPath) {
    $wheel = Get-ChildItem -Path (Join-Path $Root "dist") -Filter "xqshare-*.whl" -ErrorAction SilentlyContinue |
        Sort-Object LastWriteTime -Descending |
        Select-Object -First 1
    if (-not $wheel) {
        throw "No xqshare wheel found in dist/. Run: python -m build --wheel"
    }
    $WheelPath = $wheel.FullName
}
if (-not $OutputPath) {
    $OutputPath = Join-Path $Root "dist\xqshare-server-standalone.ps1"
}

$wheelName = Split-Path $WheelPath -Leaf
$bytes = [System.IO.File]::ReadAllBytes((Resolve-Path $WheelPath).Path)
$base64 = [Convert]::ToBase64String($bytes)
$chunks = New-Object System.Collections.Generic.List[string]
for ($i = 0; $i -lt $base64.Length; $i += 7600) {
    $len = [Math]::Min(7600, $base64.Length - $i)
...
$ErrorActionPreference = "Stop"
$EmbeddedWheelName = "__WHEEL_NAME__"
$EmbeddedWheelBase64 = @(
__WHEEL_CHUNKS__
)
...
function Write-EmbeddedWheel {
    $dir = Join-Path $env:TEMP "xqshare-standalone"
    New-Item -ItemType Directory -Force -Path $dir | Out-Null
    $path = Join-Path $dir $EmbeddedWheelName
    $joined = [string]::Concat($EmbeddedWheelBase64)
    [System.IO.File]::WriteAllBytes($path, [Convert]::FromBase64String($joined))
    return $path
}
...
if (-not $SkipPackageInstall) {
    Write-Step "Extracting embedded xqshare wheel"
    $wheelPath = Write-EmbeddedWheel
    ...
    Write-Step "Installing embedded xqshare wheel"
    Write-Host "Installing xqshare package from embedded wheel..."
...
$chunkLines = ($chunks | ForEach-Object { '    "' + $_ + '"' }) -join ",`r`n"
$content = $template.Replace("__WHEEL_NAME__", $wheelName).Replace("__WHEEL_CHUNKS__", $chunkLines)
New-Item -ItemType Directory -Force -Path (Split-Path $OutputPath -Parent) | Out-Null
[System.IO.File]::WriteAllText((Resolve-Path (Split-Path $OutputPath -Parent)).Path + "\" + (Split-Path $OutputPath -Leaf), $content, [System.Text.Encoding]::ASCII)
